...
function goalProgress(goal) {
  ...
  if (!target) return 0
  return Math.min(100, Math.round((Number(goal.current_amount || 0) / target) * 100))
}
    ...
    <div className="dashboard-grid bottom finance-goal-layout">
          ...
          {activeGoals.length ? activeGoals.map((goal) => {
            ...
            return <article className="goal-card" key={goal.id}>
              ...
              <form className="goal-update-form" action={updateFinanceGoalProgress}>
                ...
                <label>Current<input required name="current_amount" type="number" min="0" step="0.01" defaultValue={Number(goal.current_amount || 0)}/></label>
      ...
      <section className="panel new-panel">
        ...
        <form action={createFinanceGoal}>
          ...
          <div className="form-row">
            <label>Current amount (ETB)<input name="current_amount" type="number" min="0" step="0.01" defaultValue="0"/></label>
            <label>Start date<input required name="period_start" type="date" defaultValue={todayDate}/></label>
          </div>
          <label>End date<input required name="period_end" type="date" defaultValue={defaultGoalEnd}/></label>
