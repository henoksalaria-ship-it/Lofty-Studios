...
export default async function NewDealPage() {
  ...
  const [{ data: companies = [] }, { data: contacts = [] }, { data: deals = [] }] = await Promise.all([
    ...
    supabase.from('deals').select('id,company_id,deal_title,stage,companies(company_name)').eq('workspace_id', workspace.id).order('updated_at', { ascending: false }).limit(1000),
  ])
